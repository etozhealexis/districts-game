...
@Component
public class DefendDistrictCommand extends DistrictsBotCommand {
    private final DistrictsMessagingService districtsMessagingService;

    @Value("${telegram.game-chat.id}")
    private Long gameChatId;

    protected DefendDistrictCommand(DistrictsMessagingService districtsMessagingService) {
        super(Command.DEFEND.getAlias(), Command.DEFEND.getDescription());
    ...
    public void execute(AbsSender absSender, User user, Chat chat, String[] arguments) {
        ...
        LocalDateTime defendTime = LocalDateTime.now();
